...
def test_gen_customer_order_id(mocker) -> None:
    mocker.patch("src.logic.lot.utils.c_id", new="Random")
...
def test_post_lot_generation(mocker) -> None:
    mocker.patch("src.logic.lot.utils.minimum_quantity_generation", return_value=1.0)
    mocker.patch("src.logic.lot.utils.gen_customer_order_id", return_value="order")
    mocker.patch("src.logic.lot.utils.currency_pair", new="USDZAR")
    mocker.patch("src.logic.lot.utils.post_only", new=True)
    mocker.patch("src.logic.lot.utils.time_in_force", new="GTC")
    r1 = post_lot_generation(1.0, side=ConLot.buy)
    mocker.patch("src.logic.lot.utils.currency_pair", new="BTCZAR")
...
def test_minimum_quantity_generation(mocker) -> None:
    mocker.patch("src.logic.lot.utils.quantity", new="0.15")
...
def test_filtered_open_orders(mocker) -> None:
    mocker.patch("src.logic.lot.utils.get_all_open_orders", return_value=[])
    r1 = filtered_open_orders(pair="BICUSD")
    assert r1 == []
    mocker.patch(
        "src.logic.lot.utils.get_all_open_orders",
        return_value=[
            {
